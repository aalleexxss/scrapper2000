...
*/
  ...
  println("dfC1")
  var dfC1 = spark.read.format("csv").option("header", "true").load(s"D:\\Revature\\DowloadDataScala\\CSVs\\al00001.csv") // s"s3a://$bucket/time_series_covid_19_confirmed.csv
  ...
  statelist.foreach( i => { println(i)
    ...
    println("dfC2")
    var dfC2 = spark.read.format("csv").option("header", "true").load(s"D:\\Revature\\DowloadDataScala\\CSVs\\${i}00001.csv") // s"s3a://$bucket/time_series_covid_19_confirmed.csv
